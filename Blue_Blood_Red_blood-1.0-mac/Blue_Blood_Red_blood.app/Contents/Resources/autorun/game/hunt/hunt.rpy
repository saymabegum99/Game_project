...
transform q_move01:
    ...
    xanchor 200
    ##change
    ##this is to move the cross-hair (code below), moving target will be later.
    linear 1.5 xpos 900
    linear 3.0 xpos 0
    linear 1.5 xpos 350
    repeat
...
transform mark01 (id):
    ## spaces out the green and red squares for hit or miss.
    xpos  id * 30
    ypos 0
...
label begin_hunt01:
    ...
    ## calls this function to the main script file in label gun1
    ...
    while shots_fired01 < 3:
        ...
        call hunting01 from _call_hunting01
        ...
        if _return: # Good result
            $ targets_hit01 += 1
            $ renpy.show ('', at_list=[mark01(shots_fired01)], layer='master',
            what=Solid('#00FF00', xsize=24, ysize=24), zorder=50,
            tag='mark01%d'%shots_fired01)

        else:       # bad result
            $ renpy.show ('', at_list=[mark01(shots_fired01)],
            layer='master', what=Solid('#FF0000', xsize=24, ysize=24),
            zorder=50, tag='mark01%d'%shots_fired01)
